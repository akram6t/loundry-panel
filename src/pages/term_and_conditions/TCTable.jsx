...
function TCTable({ edit, onRefresh, collection, loading, dataHeader, data, currentPage, itemsPerPage }) {
      ...
      {data?.map((row, index) => (
        ...
        >
          ...
          <TableCell dataLabel="TITLE" showLabel={true}>
            <span className="font-semibold text-sm text-gray-900">{row.title}</span>
          </TableCell>
          <TableCell dataLabel="DESCRIPTION" showLabel={true}>
            <span className="font-medium text-sm text-gray-900">{row.description.length > 50 ? row.description.slice(0, 50)+'.....' : row.description}</span>
          </TableCell>
          <TableCell dataLabel="STATUS" showLabel={true}>
              <span
                className={`font-medium text-sm text-gray-900 ${status.find((item) => item.label === row.status).color
          ...
          <TableCell dataLabel="ACTIONS" showLabel={true}>
            ...
            <button
              ...
              <button
                  onClick={() => deleteData(row._id, row.title)}
